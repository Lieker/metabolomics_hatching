...
plotpca(x = exudate_neg_frt) + scale_size(guide = "none")
plotpca(x = exudate_pos_frt) + scale_size(guide = "none")
```
...
#get corr dfs
source("scripts/ordermatrixforcorplot.R")
c_extp_sol <- ordermatrixforcorplot(x = t(extract_pos_f), interest = "solA", compounds = c_pos_extr)
c_extn_sol <- ordermatrixforcorplot(x = t(extract_neg_f), interest = "solA", compounds = c_neg_extr)

#plot corplot
source("scripts/plotcor.R")
testRes = cor.mtest(c_extp_sol, conf.level = 0.95)
corplot_extp_sol <- plotcor(x = c_extp_sol)
testRes = cor.mtest(c_extn_sol, conf.level = 0.95)
corplot_extn_sol <- plotcor(x = c_extn_sol)
```
...
#plot corplot
source("scripts/plotcor.R")
testRes = cor.mtest(c_extp_hatching, conf.level = 0.95)
corplot_extp_hatching <- plotcor(x = c_extp_hatching)
testRes = cor.mtest(c_extn_hatching, conf.level = 0.95)
corplot_extn_hatching <- plotcor(x = c_extn_hatching)
```