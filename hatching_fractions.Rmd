...
fr_n_subset$genotype <- rep(c(rep("Avatar", 4), rep("Desiree", 4), rep("Seresta", 4)), 3)
fr_n_subset$fraction <- rep(c("F08","F09","F10","F11"), 9)
...
pn <- ggplot(fr_n_subset, aes(x = fraction, y = value)) +
  ...
  ylab("counts") +
  scale_colour_discrete(name = "feature", labels = c("m/z 497.11, Rt 7.67", "m/z 497.18, Rt 6.70", "m/z 525.18, Rt 7.32")) +
...
pp <- ggplot(fr_p_subset, aes(x = fraction, y = value)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.x=element_blank()) +
  ylab("counts") +
  scale_colour_discrete(name = "feature", labels = c("m/z 499.20, Rt 6.69")) +
  ggtitle("Positive mode") +
  facet_grid(. ~ genotype)
pp

fr_subset <- rbind(fr_n_subset, fr_p_subset)
fr_subset$mode <- c(rep("negative", 36), rep("positive", 12))
fr_subset$label <- c(rep("m/z 497.11 (-), Rt 7.67", 12), rep("m/z 497.18 (-), Rt 6.70", 12), rep("m/z 525.18 (-), Rt 7.32", 12), rep("m/z 499.20 (+), Rt 6.69", 12))
...
ppn <- ggplot(fr_subset, aes(x = fraction, y = value)) +
  geom_line(aes(group = label, color = label), size = 1) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.x=element_blank()) +
  ylab("counts") +
  ...
  geom_bar(data = hatch_s_subset,
               ...
               fill = "gray",
           alpha = 0.5) +
  scale_y_continuous(sec.axis = sec_axis(trans=~./37.5,
                                         name="hatching (%)",
...
testRes = cor.mtest(cor_fr_p, conf.level = 0.95)
corplot_fr_p <- plotcor(x = cor_fr_p)
...
source("scripts/plotcor.R")
testRes = cor.mtest(cor_fr_n, conf.level = 0.95)
corplot_fr_n <- plotcor(x = cor_fr_n)
```
